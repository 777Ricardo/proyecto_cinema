...
?>
<?php
$vardistribuidoresporpelicula_DatosActualizar = "0";
if (isset($_GET["distribuidoresporpelicula"])) {
  $vardistribuidoresporpelicula_DatosActualizar = $_GET["distribuidoresporpelicula"];
}

$query_DatosActualizar = sprintf("SELECT * FROM distribuidoresporpelicula WHERE distribuidoresporpelicula.idDistrbuidora = %s", GetSQLValueString($vardistribuidoresporpelicula_DatosActualizar, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE distribuidoresporpelicula SET idPelicula=%s WHERE idDistrbuidora=%s",
                       GetSQLValueString($_POST['idPelicula'], "int"),
					   GetSQLValueString($_POST['idDistribuidora'], "int"));

  
  $Result1 = mysqli_query($conexioncine,$updateSQL) or die(mysqli_error($conexioncine));

  $updateGoTo = "distribuidoresporpelicula.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<title>Actualizar Información de los Distribuidores por Pelicula</title>
<link rel="stylesheet" href="css/estilos.css" type="text/css"/>
</head>
<body>
<br><br><br>
<h1 align="center">ACTUALIZAR INFORMACION DE LOS DISTRIBUIDORES POR PELICULA</h1>
  <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
    ...
    <input type="hidden" name="idDistrbuidora" value="<?php echo $row_DatosActualizar['idDistrbuidora']; 
												  ?>"/>
</form>
</body>
</html>