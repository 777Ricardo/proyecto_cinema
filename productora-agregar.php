...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "formregistro")) {
  $insertSQL = sprintf("INSERT INTO productora (idProductora, productora) VALUES (%s, %s)",
					   
					   GetSQLValueString($_POST['idProductora'], "int"),
                       GetSQLValueString($_POST['productora'], "text"));
	

  
  $Result1 = mysqli_query($conexioncine,$insertSQL) or die(mysqli_error($conexioncine));

  $insertGoTo = "productora.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<title>Ingresar Nueva Productora</title>
<link rel="stylesheet" href="css/estilos.css"  type="text/css" />
</head>
<body>
<br><br><br><br>
<h1 align="center">INGRESAR NUEVA PRODUCTORA</h1>
  
  <form action="<?php echo $editFormAction; ?>" method="post" name="formregistro" id="formregistro" onsubmit="return validateformregistro();">
    <table align="center">
		
		<tr valign="baseline">
        <td nowrap="nowrap" align="right">Id Productora:</td>
        <td><input type="text" name="idProductora" value="" size="32" class="campo" /></td>
      </tr>
		
      <tr valign="baseline">
        <td nowrap="nowrap" align="right">Productora:</td>
        <td><input type="text" name="productora" value="" size="32" class="campo" /></td>
      </tr>
