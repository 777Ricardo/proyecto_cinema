...
?>
<?php
$varactores_DatosActualizar = "0";
if (isset($_GET["actores"])) {
  $varactores_DatosActualizar = $_GET["actores"];
}
$query_DatosActualizar = sprintf("SELECT * FROM actores WHERE actores.idActores = %s", GetSQLValueString($varactores_DatosActualizar, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE actores SET apellidos=%s, nombres=%s, imagen=%s WHERE idActores=%s",
                       
					   GetSQLValueString($_POST['apellidos'], "text"),
					   GetSQLValueString($_POST['nombres'], "text"),
					   GetSQLValueString($_POST['imagen'], "text"),
                       GetSQLValueString($_POST['idActores'], "int"));

  
  $Result1 = mysqli_query($conexioncine,$updateSQL) or die(mysqli_error($conexioncine));

  $updateGoTo = "actores.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<title>Actualizar Información de Actores </title>
<link rel="stylesheet" href="css/estilos.css" type="text/css"/>
</head>
<body>
<br><br><br>
<h1 align="center">ACTUALIZAR INFORMACION DE ACTORES </h1>
  <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
    <table align="center">
        ...
        <td><input type="text" name="apellidos" class="campo" value=" <?php echo utf8_encode($row_DatosActualizar['apellidos']); ?>" size="32" /></td>
      </tr>
       
       <tr valign="baseline">
        <td nowrap="nowrap" align="right">Nombres:</td>
        <td><input type="text" name="nombres" class="campo" value=" <?php echo utf8_encode($row_DatosActualizar['nombres']); ?>" size="32" /></td>
      </tr>
    ...
    <input type="hidden" name="idActores" value="<?php echo $row_DatosActualizar['idActores']; 
												  ?>"/>
</form>
</body>
</html>
